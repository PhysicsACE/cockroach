...
func (d *SubscriptAccessExpr) EvalAccess(ctx *JsonPathEvalCtx, j json.JSON) (json.JSON, error) {
	...
	if !j.IsJsonArray() {
		return nil, errors.New("jsonpath: subscript access on non-array")
	}
...
func (d *RecursiveWildcardAccessExpr) EvalAccess(ctx *JsonPathEvalCtx, j json.JSON) (json.JSON, error) {
	if d.levels != nil {
		start := d.levels.Begin.Eval(ctx)
		levelBegin := 0
		if start.IsJsonNumber() {
			startNum, _ := json.AsJsonNumber(start)
			levelBegin, startErr := startNum.Int64()
			if startErr != nil {
				return nil, startErr
			}
		}

		end := d.levels.End.Eval(ctx)
		levelEnd := 0
		if end.IsJsonNumber() {
			endNum, _ := json.AsJsonNumber(end)
			levelEnd, endErr := endNum.Int64()
			if endErr != nil {
				return nil, endErr
			}
		}

		flattened := json.FlattenJson(j, true, levelBegin, levelEnd)
		return flattened, nil
	} else {
		flattened := json.FlattenJson(j, false, 0, 0)
		return flattened, nil
	}
}
