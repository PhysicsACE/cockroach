...
func (b *logicalPropsBuilder) buildScanProps(scan *ScanExpr, rel *props.Relational) {
	...
	if hardLimit == 1 {
		...
		// Scan operator. If the scan is an inverted index scan, then the table
		// FDs should not be used because each logical row can correspond to
		// multiple tuples in the inverted index.
		//
		// TODO(#122225): Note that we currently copy table FDs for inverted
		// scans with inverted constraints, i.e., all inverted scans that are
		// not generated by GenerateTrigramSimilarityInvertedIndexScans. We
		// should not be doing this because these FDs are not correct because
		// scans over inverted indexes can produce multiple tuples for each
		// logical row in the table. We maintain this behavior for now for
		// historical reasons - it's been "broken" since inverted scans were
		// introduced. It has potential to cause incorrect results, though there
		// are no known correctness bugs, likely because there are no optimizer
		// rules that operate on inverted filter expressions. We should fix this
		// to prevent potential bugs in the future by not copying the table FDs
		// here and adding primary keys to the FDs of inverted filters.
		if !scan.IsInvertedScan(md) || scan.InvertedConstraint != nil {
